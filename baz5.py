...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mBasari_ID_xy')
...
def menu(my_name,my_id):
	...
	print('[><] 3. Crack Dari group member ')
	print('[><] 4. Crack Dari File	')
	print('[><] 5. Cek Hasil Crack  ')
	print('[><] 0. Keluar       ')
	_____Muhammad__Basari_____ = input('\n[•] Pilih : ')
	if _____muhammad__basari_____ in ['1']:
		dump_massal()
...
def grup():
	print('')
	id = input(f'{x}[•] Masukkan ID Grup Target : ')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/BASARI-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/BASARI-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/BASARI-DUMP/'+geh,'r').read().splitlines()
		except:
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('--> Tampilkan Aplikasi Terkait ( Y/t ) ')
	if _jembot_ in ['']:
		print('--> Pilih Yang Bener Kontol ')
		back()
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] anjing,ngentod,bangsat[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'------> {m}•{k}•{h}•{x} Semoga Mendapatkan Hasil {m}•{k}•{h}•{x} <------ ')
	print('')
	print(f'-->AKUN {h}OK{x} TERSIMPAN DI : {h}OK/%s {x}'%(okc))
	print(f'--> AKUN {k}CP{x} TERSIMPAN DI : {k}CP/%s {x}'%(cpc))
	print(f'--> ON OF MODE PESAWAT SETIAP {m}1K{x} ID\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rʕ•ᴥ•ʔ {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/BASARI-DUMP')
	except:pass
	...
	alvino_xy(f'\n\t{x}——> {h}selanat datang tod semoga harimu menyenangkan{x}')
	time.sleep(3)
	login()
