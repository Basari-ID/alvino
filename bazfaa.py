...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mputra_basari_xy')
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tAmbil Cookies Dari [green]Kiwi Browser[white]'))
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'1. Hasil Akun {h}OK{x} Anda ')
	print(f'2. Hasil Akun {k}CP{x} Anda ')
	print('3. Kembali	')
	kz = input(f'\n[?] Pilih Mana : ')
	if kz in ['2']:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}1. Crack Akun Old ')
	print('2. Crack Akun New ')
	print('')
	hu = input('[?] Pilih Mana : ')
	if hu in ['1','01']:
...
def passwrd():
	...
	print('')
	print('Ingin Lanjut Crack Kembali ( Y/t ) ? ')
	woi = input('--> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r(´･_･`) {P}[{b}{loop}{P}/{u}{len(id)}{P}]OK{P}[{H}{ok}{P}]CP{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}[><] {h} Selamat Datang Cuy >< Info Lebih Lanjut Chat Fbku [><]{x}')
	time.sleep(3)
	login()
